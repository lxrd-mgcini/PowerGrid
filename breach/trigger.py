...
# This is a simple script that allows an operator to send a TCP or UDP packet at a system with a specified DATA portion. It is used in Veil PowerTools to trigger the PSBreach backdoors that require a network trigger.
import argparse
import socket
import time
...
parser.add_argument("-m", "--method", type=int, choices=[
                    0, 1, 2], default=0, help="Select method of trigger: 1(UDP), 2(TCP Bind)")
parser.add_argument("-p", "--port", type=int, default=4444,
                    help="Port number to trigger on")
